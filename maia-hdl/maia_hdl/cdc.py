...
def gen_verilog_txiq():
    m = Module()
    internal = ClockDomain()
    m.domains += internal
    m.submodules.dump = dump = TxIQCDC('sync', 'internal', 18)
    with open('txiq_cdc.v', 'w') as f:
        f.write(amaranth.back.verilog.convert(
            m, ports=[
                dump.re_in, dump.im_in, dump.reset, dump.strobe_in, dump.valid_re, dump.valid_im,
